...
2.  -Null Hypothesis (H0): There is no difference in the mean number of boys and girls born each year.
    -Alternative hypothesis(H1): There is a difference in the mean number of boys and girls born each year.

    

3.  Permutation Test
  - Shuffle the gender labels each year to simulate the null distribution of birth counts under the assumption of no difference between boys and girls.
  -Calculate the observed difference in mean birth counts
  -Compare the observed difference to the distribution of permuted differences to estimate a p-value.
...
```
The histogram represents the distribution of differences in means (boys - girls) under the null hypothesis, where the gender labels were randomly shuffled.
The distribution is approximately centered around zero, which is expected if there is no real difference between the two groups (boys and girls).
...
Source of Data: https://github.com/OpenIntroStat/openintro/blob/HEAD/R/data-present.R
